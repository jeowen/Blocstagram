            ...
            if (granted) {
                ...
                if (!input) {
                   ...
                   // [self presentViewController:alertVC animated:YES completion:nil];
                    [self alertMessage:error.localizedDescription message:error.localizedRecoverySuggestion];
                } else {
                    // #7
               ...
               //                                                                  message:NSLocalizedString(@"This app doesn't have permission to use the camera; please update your privacy settings.", @"camera permission denied recovery suggestion")
                ...
                // [self presentViewController:alertVC animated:YES completion:nil];
                [self alertMessage:NSLocalizedString(@"Camera Permission Denied", @"camera permission denied title") message:NSLocalizedString(@"This app doesn't have permission to use the camera; please update your privacy settings.", @"camera permission denied recovery suggestion")];
            }
        });
    }];
}
...
// EXPERIMENT 2
- (void) alertMessage:(NSString *)title message:(NSString *)messageReceived {
    
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:title message:messageReceived preferredStyle:UIAlertControllerStyleAlert];
    [alertVC addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"OK button") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
        [self.delegate cameraViewController:self didCompleteWithImage:nil];
    }]];
